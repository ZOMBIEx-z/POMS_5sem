...
public class Main {
    ...
    private static final String DEFAULT_FILENAME = "fleet.dat";
...
//        }
    ...
    private static void displayMenu() {
        ...
        while (choice != 0) {
            ...
            System.out.println("6. Сохранить таксопарк в файл");
            System.out.println("7. Загрузить таксопарк из файла");
    ...
    private static void saveFleetToFile() {
        System.out.print("Введите имя файла для сохранения (Enter для " + DEFAULT_FILENAME + "): ");
        String input = scanner.nextLine().trim();
        String filename = input.isEmpty() ? DEFAULT_FILENAME : input;

        try {
            FleetStorage.save(fleet, filename);
            System.out.println("Таксопарк сохранён в файл: " + filename);
        } catch (IOException e) {
            System.out.println("Ошибка при сохранении файла: " + e.getMessage());
        }
    }

    private static void loadFleetFromFile() {
        System.out.print("Введите имя файла для загрузки (Enter для " + DEFAULT_FILENAME + "): ");
        String input = scanner.nextLine().trim();
        String filename = input.isEmpty() ? DEFAULT_FILENAME : input;

        try {
            fleet = FleetStorage.load(filename);

            System.out.println("Таксопарк загружен из файла: " + filename);
            System.out.println(fleet);
